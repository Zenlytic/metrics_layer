...
# , Definitions.bigquery, Definitions.redshift])
...
@pytest.mark.query
@pytest.mark.parametrize("query_type", [Definitions.snowflake])
def test_orders_funnel_query(connection, query_type):
    ...
    query = connection.get_sql_query(metrics=["number_of_orders"], funnel=funnel, query_type=query_type)
    ...
    correct = (
        ...
        "ON order_lines.customer_id=customers.customer_id) ,step_1 AS ("
        ...
        "WHERE base.order_lines_channel IN ('Organic','Email') AND DATEDIFF('DAY', "
        "step_1.step_1_time, base.orders_order_raw) <= 3) ,result_cte AS ("
