...
def test_seed_single_view_returns_view_dict(
    ...
    if query_type in {Definitions.snowflake, Definitions.databricks, Definitions.redshift}:
    ...
    def run_query_mock(query):
        call_count["n"] += 1
        q = query.upper()
        if ".COLUMNS" in q or "COLUMN_FIELD_PATHS" in q or "SVV_COLUMNS" in q:
            return table_data
        if ".TABLES" in q or "SVV_TABLES" in q:
    ...
    # Table comment should be set for Snowflake/Databricks/Redshift
    if query_type in {Definitions.snowflake, Definitions.databricks, Definitions.redshift}:
