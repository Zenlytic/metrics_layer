...
def test_access_filters_on_view(connection):
    ...
    correct = (
        "(select * from analytics.submitted_form WHERE submitted_form.id='Airplant Holder') as submitted_form"
    )
    ...
    result = view.secure_from_statement(query_type="SNOWFLAKE")
    correct = (
        "(select * from analytics.submitted_form WHERE submitted_form.id IN ('Airplant"
...
def test_access_filters_on_view_with_always_filter_and_access_filters(connection):
    ...
    result = view.secure_from_statement(query_type="SNOWFLAKE")
    correct = (
        "(select * from analytics.query_in_workspace WHERE query_in_workspace.context_os='iOS' and NOT"
    ...
    result = view.secure_from_statement(query_type="SNOWFLAKE")
    correct = (
        "(select * from analytics.query_in_workspace WHERE query_in_workspace.context_os='Android' and"
        " query_in_workspace.customer_id IN ('1123','4434') and NOT query_in_workspace.context_os IS NULL and"
    ...
    result = view.secure_from_statement(query_type="SNOWFLAKE")
    correct = (
        "(select * from analytics.query_in_workspace WHERE NOT query_in_workspace.context_os IS NULL and"
        " query_in_workspace.context_os IN ('1','Google','os:iOS') and query_in_workspace.session_id NOT IN"
        " (1,44,87)) as query_in_workspace"
    )
    assert result == correct
...
def test_access_filters_on_view_derived_table(connection):
    ...
    result = view.secure_from_statement(query_type="SNOWFLAKE")
    correct = (
        "(select * from (SELECT * FROM ANALYTICS.COUNTRY_DETAIL WHERE 'Europe' = COUNTRY_DETAIL.REGION) WHERE"
        " country_detail.country='US-West') as country_detail"
    )
    assert result == correct
