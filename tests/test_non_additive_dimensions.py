...
def test_mrr_non_additive_dimension_no_group_by_max(connection, metric_suffix):
    ...
    correct = (
        f"WITH cte_mrr_{metric_suffix}_record_raw AS (SELECT {func}(mrr.record_date) as mrr_{func.lower()}_record_raw "
        f"FROM analytics.mrr_by_customer mrr ORDER BY mrr_{func.lower()}_record_raw DESC) "
        f"SELECT SUM(case when mrr.record_date=cte_mrr_{metric_suffix}_record_raw.mrr_{metric_suffix}_{func.lower()}_record_raw "
...
def test_mrr_non_additive_dimension_no_group_by_with_where(connection):
    ...
    correct = (
        ...
        f"SELECT SUM(case when mrr.record_date=cte_mrr_end_of_month_record_raw.mrr_end_of_month_max_record_raw "
...
def test_mrr_non_additive_dimension_no_group_by_with_window_grouping(connection):
    ...
    correct = (
        ...
        ".mrr_end_of_month_by_account_max_record_date "
        ...
        "=cte_mrr_end_of_month_by_account_record_date.mrr_account_id ORDER BY mrr_mrr_end_of_month_by_account DESC;"
    )
    assert query == correct
...
def test_mrr_non_additive_dimension_time_group_by(connection):
    ...
    correct = (
        "WITH cte_mrr_end_of_month_record_raw AS (SELECT DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) as mrr_record_week,"
        "MAX(mrr.record_date) as mrr_max_record_raw "
        "FROM analytics.mrr_by_customer mrr GROUP BY DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) ORDER BY mrr_max_record_raw DESC) "
        "SELECT DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) as mrr_record_week,"
        "SUM(case when mrr.record_date=cte_mrr_end_of_month_record_raw.mrr_end_of_month_max_record_raw "
        "then mrr.mrr end) as mrr_mrr_end_of_month FROM analytics.mrr_by_customer mrr "
        "JOIN cte_mrr_end_of_month_record_raw ON DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE))=cte_mrr_end_of_month_record_raw.mrr_record_week "
...
def test_mrr_non_additive_dimension_time_group_by_with_where(connection):
    ...
    correct = (
        ...
        "AND DATE_TRUNC('DAY', mrr.record_date)>'2022-04-03' GROUP BY DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) ORDER BY mrr_max_record_raw DESC) "
        "SELECT DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) as mrr_record_week,"
        "SUM(case when mrr.record_date=cte_mrr_end_of_month_record_raw.mrr_end_of_month_max_record_raw "
        "then mrr.mrr end) as mrr_mrr_end_of_month FROM analytics.mrr_by_customer mrr "
        "JOIN cte_mrr_end_of_month_record_raw ON DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE))=cte_mrr_end_of_month_record_raw.mrr_record_week "
...
def test_mrr_non_additive_dimension_time_group_by_with_window_grouping(connection):
    ...
    correct = (
        f"WITH cte_mrr_end_of_month_by_account_record_date AS (SELECT mrr.account_id as mrr_account_id,DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) as mrr_record_week,"
        "MAX(DATE_TRUNC('DAY', mrr.record_date)) as mrr_max_record_date "
        f"FROM analytics.mrr_by_customer mrr GROUP BY mrr.account_id,DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE)) ORDER BY mrr_max_record_date DESC) "
        ...
        f"then mrr.mrr end) as mrr_mrr_end_of_month_by_account FROM analytics.mrr_by_customer mrr "
        f"JOIN cte_mrr_end_of_month_by_account_record_date ON mrr.account_id"
        "=cte_mrr_end_of_month_by_account_record_date.mrr_account_id and DATE_TRUNC('WEEK', CAST(mrr.record_date AS DATE))"
        "=cte_mrr_end_of_month_by_account_record_date.mrr_record_week "
...
def test_mrr_non_additive_dimension_alt_group_by_with_where(connection):
    ...
    correct = (
        ...
        "MAX(mrr.record_date) as mrr_max_record_raw "
        "FROM analytics.mrr_by_customer mrr WHERE mrr.plan_name='Enterprise' "
        "AND DATE_TRUNC('DAY', mrr.record_date)>'2022-04-03' GROUP BY mrr.plan_name ORDER BY mrr_max_record_raw DESC) "
        "SELECT mrr.plan_name as mrr_plan_name,"
        "SUM(case when mrr.record_date=cte_mrr_end_of_month_record_raw.mrr_end_of_month_max_record_raw "
        "then mrr.mrr end) as mrr_mrr_end_of_month FROM analytics.mrr_by_customer mrr "
        "JOIN cte_mrr_end_of_month_record_raw ON mrr.plan_name=cte_mrr_end_of_month_record_raw.mrr_plan_name "
...
def test_mrr_non_additive_dimension_alt_group_by_with_having(connection):
    ...
    correct = (
        ...
        "HAVING SUM(case when mrr.record_date=cte_mrr_end_of_month_record_raw.mrr_end_of_month_max_record_raw then mrr.mrr end)>1100 "
...
def test_mrr_non_additive_dimension_alt_group_by_with_window_grouping(connection):
    ...
    correct = (
        f"WITH cte_mrr_end_of_month_by_account_record_date AS (SELECT mrr.account_id as mrr_account_id,mrr.plan_name as mrr_plan_name,"
        "MAX(DATE_TRUNC('DAY', mrr.record_date)) as mrr_max_record_date "
        f"FROM analytics.mrr_by_customer mrr GROUP BY mrr.account_id,mrr.plan_name ORDER BY mrr_max_record_date DESC) "
...
def test_mrr_non_additive_dimension_group_by_equal_to_window_grouping(connection):
    ...
    correct = (
        ...
        f"then mrr.mrr end) as mrr_mrr_end_of_month_by_account FROM analytics.mrr_by_customer mrr "
        f"JOIN cte_mrr_end_of_month_by_account_record_date ON mrr.account_id"
        "=cte_mrr_end_of_month_by_account_record_date.mrr_account_id "
...
def test_mrr_non_additive_dimension_merged_results_no_group_by_where(connection):
    ...
    correct = (
        ...
        "mrr_end_of_month_max_record_raw then mrr.mrr end) as mrr_mrr_end_of_month "
...
def test_mrr_non_additive_dimension_merged_results_no_group_by_window_grouping(connection):
    ...
    correct = (
        ...
        f"then mrr.mrr end) as mrr_mrr_end_of_month_by_account FROM analytics.mrr_by_customer mrr "
        f"JOIN cte_mrr_end_of_month_by_account_record_date ON mrr.account_id"
        "=cte_mrr_end_of_month_by_account_record_date.mrr_account_id WHERE DATE_TRUNC('DAY', mrr.record_date)>'2022-04-03' "
        "ORDER BY mrr_mrr_end_of_month_by_account DESC"
        ") ,order_lines_order__cte_subquery_1 AS (SELECT SUM(order_lines.revenue) as "
        "order_lines_total_item_revenue FROM analytics.order_line_items order_lines "
        "WHERE DATE_TRUNC('DAY', order_lines.order_date)>'2022-04-03' ORDER BY "
        "order_lines_total_item_revenue DESC) SELECT mrr_record__cte_subquery_0.mrr_mrr_end_of_month_by_account "
...
def test_mrr_non_additive_dimension_merged_results_time_group_by(connection):
    ...
    correct = (
        ...
        "ON mrr_record__cte_subquery_0.mrr_record_week=order_lines_order__cte_subquery_1.order_lines_order_week;"
    )
    assert query == correct
